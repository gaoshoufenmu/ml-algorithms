...
$$d^* = max_{\lambda, \mu} \ \theta_D(\lambda, \mu) \le min_x \ \theta_P (x) = p^*$$                                                                                 \(6\)

然而我们仅仅得到上式这个关系还不够，还是无法证明对偶问题的最佳解就是原始问题的最佳解。

\(6\)式的关系表明一种弱对偶性，存在一个对偶间隙\(duality gap\) $$p^* - d^*$$，如果\(6\)式等式成立，则称强对偶性，此时对偶问题的最佳解与原始问题的最佳解一致，但是这里不给出证明（其实我不会说这个证明不太容易所以才略去的）。

### Complementary Slackness

设原始问题$$\theta_P(x)$$的最佳解为$$x^*$$，对偶问题$$\theta_D(\lambda, \mu)$$的最佳解为$$(\lambda^*, \mu^*)$$，那么，

$$\theta(\lambda^*, \mu^*) = inf_x [L(x, \lambda^*, \mu^*)] \le f(x^*) + \sum_{j=1}^p \lambda_j^* h_j(x^*) + \sum_{k=1}^q \mu_k^* g_k(x^*) \le f(x^*)$$    \(7\)

最后一个不等号成立是由于约束条件。若对偶问题满足强对偶性，那么，

$$\theta(\lambda^*, \mu^*) = d^* = p^* = f(x^*)$$

所以结合\(7\)式，需要满足，

$$\begin{cases} \sum_{j=1}^p \lambda_j^* h_j(x^*) = 0 \quad (a) \\ \sum_{k=1}^q \mu_k^* g_k(x^*) = 0 \quad (b) \end{cases}$$

根据原始优化问题的约束条件，\(a\)式满足是必然的，然而要\(b\)满足的话，那么，

$$\mu_k^* g_k(x^*) = 0 \quad \forall k \in [q]$$

也就是说$$\mu_k^*$$ 和$$g_k(x^*)$$ 至少有一个为0。

### KKT

综上，我们得到可以通过求解对偶问题的最佳解来获取的原始优化问题的最佳解的条件（称为KKT条件），
...
\(2\) $$\begin{cases} h_j(x^*) = 0, \quad \forall j \in [p] \\ g_k(x^*) \le 0, \quad \forall k \in [q] \end{cases}$$

原始优化问题的约束条件

\(3\)$$\mu_k^* \ge 0, \quad \forall k \in [q]$$

原始问题转化为对偶问题的条件

\(4\)$$\mu_k^* g_k(x^*) = 0 \quad \forall k \in [q]$$

强对偶性条件，使得原始问题与对偶问题有相同解。
