...
$$H(Y|X) = \sum_{i=1} p_i H(Y|X=x_i)$$

而依葫芦画瓢，下面的推导不难，

$$H(Y|X = x_i) = - \sum_{j=1}^m (P(Y = y_j|X = x_i) log P(Y = y_j|X = x_i))$$

将$$P(Y = y_j|X = x_i)$$ 简记为$$p\_{j|i}$$，于是，

$$H(Y|X) = -\sum_{i=1}^n \sum_{j=1}^m p_i p_{j|i} log p_{j|i} = - \sum_{i=1}^n \sum_{j=1}^m p_{ij} log p_{j|i}$$
...
$$g_R(D, A) = p \cdot g(D, A)$$

其中$$p$$为惩罚参数，通常是将某个特征$$A$$看作随机变量，按照其特征取值对数据集$$D$$划分，计算熵$$H_A(D)$$，由于对特征值数目较多的特征惩罚力度较大（因为其泛化能力不强），所以取熵$$H_A(D)$$的倒数，于是，

$$g_R(D, A) = \frac {g(D, A)} {H_A(D)}$$

$$H_A(D) = - \sum{i=1}^V \frac {|D_i|} {|D|} log \frac {|D_i|} {|D|} $$
